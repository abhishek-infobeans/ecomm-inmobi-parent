...
class Product {
  ...
  getProductsByCategories = async id => {
    ...
    this.state.allProducts.sort((a, b) => (a.price > b.price ? 1 : -1));
    if (id === null) {
      this.state.searchedProducts = this.state.allProducts;
    } else {
      this.state.searchedProducts = this.state.allProducts.filter(
        x => x.category === id,
      );
    }
  };

  getProductsByCategoriesShuffle = async id => {
    if (id === null) {
